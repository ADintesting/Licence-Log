...
def list_select_products(filename):
    df = pd.read_csv(filename)

    list_products = df["product"].unique()
    ...
    product_list = Listbox(window, selectmode="multiple")
    product_list.grid(column=0, row=1)
    ...
    plot_button = Button(text=f"Analyse file", width=15,
                         command=lambda: plot_licences(filename, get_selected_products(product_list), window))
    plot_button.grid(column=2, row=2, sticky='e')
...
def plot_licences(filename, products, window):
    # csv file generated by analyse function and uses pandas to create graphs
    df = pd.read_csv(filename)
    df_selected = df.query(f"product in {products}")

    # Takes a list of pandas dataframe objects and produces a plot of number of licences being used over time
    product_dfs = df_selected.groupby("product")
    num_products = product_dfs.ngroups

    df_selected['datetime'] = pd.to_datetime(df["date"] + " " + df_selected["time"], format="%d/%m/%Y %H:%M:%S")

    pivot = pd.pivot_table(df_selected, index="datetime", values='licence', columns='product', fill_value=0).cumsum().reset_index()
...
def analyse(file, window):
    ...
    list_select_products(log_csv_file.name)
